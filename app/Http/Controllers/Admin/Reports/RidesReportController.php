...
public function index(Request $request)
{
    ...
    // Base por fecha (calificar columnas)
    $qBase = Ride::query()
    ...
    $ops = (clone $qBase)->where('rides.status','finished')
        ->selectRaw('AVG(rides.duration_s) AS avg_duration_s, AVG(rides.distance_m) AS avg_distance_m')
        ->first();

    // Listado (joins + filtro de estado calificado)
    $qList = (clone $qBase)
    ...
    $rides = $qList->orderByDesc('rides.requested_at')
        ->paginate(20, [
            ...
            'rides.origin_label','rides.dest_label','rides.origin_lat','rides.origin_lng','rides.dest_lat','rides.dest_lng',
            ...
            DB::raw('d.name as driver_name'), DB::raw('d.phone as driver_phone'),
            DB::raw('v.economico as vehicle_economico'), DB::raw('v.plate as vehicle_plate'),
        ])->withQueryString();

    // Armar totals con todas las llaves que usa la vista
    ...
    return view('admin.reports.rides.index', [
        ...
        'totals' => [
            ...
            // KPIs de ingresos (lo que muestra tu UI)
            'sum_quote'          => $sumQuote,
            'avg_quote'          => $avgQuote,
  ...
  public function show(Request $request, Ride $ride)
{
    ...
    // Carga entidades
    $driver  = $ride->driver_id  ? Driver::select('id','name','phone','email')->find($ride->driver_id) : null;
    $vehicle = $ride->vehicle_id ? Vehicle::select('id','economico','plate','brand','model','color','year')->find($ride->vehicle_id) : null;

    // Segmentación de breadcrumbs + viaWave + fallback meta
    if ($ride->driver_id && $start && $end) {
        ...
        if ($acceptedAt && $arrivedAt) {
            $crumbsToPickup = $breadcrumbs->whereBetween('reported_at', [$acceptedAt, $arrivedAt])->values();
        }
        if ($onboardAt && $finishedAt) {
            $crumbsOnTrip = $breadcrumbs->whereBetween('reported_at', [$onboardAt, $finishedAt])->values();
        }
        ...
        // Fallback de vehículo si no está relacionado
        $fallbackVehicle = [
    ...
    public function exportCsv(Request $request): StreamedResponse
    {
        ...
        return response()->stream(function () use ($q) {
            ...
            $q->chunk(500, function ($rows) use ($out) {
                foreach ($rows as $r) {
                    fputcsv($out, [
                        $r->id, $r->status, $r->requested_at, $r->accepted_at, $r->arrived_at, $r->onboard_at,
                        $r->finished_at, $r->canceled_at, $r->origin_label, $r->dest_label, $r->distance_m,
                        $r->duration_s, $r->quoted_amount, $r->total_amount, $r->currency, $r->canceled_by, $r->cancel_reason
                    ]);
                }
            });
