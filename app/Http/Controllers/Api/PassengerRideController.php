...
class PassengerRideController extends Controller
{
 ...
 private const ACTIVE_STATUSES = [
    ...
    private function resolvePassengerOr422(string $firebaseUid)
    {
    ...
    private function baseCurrentRideQuery(int $passengerId)
    {
        return DB::table('rides')
            ...
            ->whereIn('rides.status', self::ACTIVE_STATUSES)
    ...
    private function buildRidePayload(object $row): array
    {
        ...
        // OJO: si tu computeRideSearchExpiresAt ya contempla nulls/estados, perfecto.
        $searchExpiresAt = $this->computeRideSearchExpiresAt($tenantId, (int)$row->id);

        $tenantBadge = $this->buildTenantBadgeFromRow($row);
        ...
        return [
            ...
            'payment_method'   => $row->payment_method ? (string)$row->payment_method : 'cash',
            ...
            'driver_id'        => $driverId,
            'has_driver'       => $driverId !== null, // ✅ mejor que !empty()
    ...
    public function current(Request $req)
    {
        ...
        $firebaseUid = (string) $v['firebase_uid'];

        $passenger = $this->resolvePassengerOr422($firebaseUid);

        $row = $this->baseCurrentRideQuery($passenger->id)
        ...
        return response()->json([
            'ok'   => true,
            'ride' => $this->buildRidePayload($row),
        ]);
    }
        ...
        public function currentAny(Request $req)
    {
        ...
        $row = $this->baseCurrentRideQuery($passenger->id)->first();
