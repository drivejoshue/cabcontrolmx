...
wave_flow: BEGIN
  ...
  IF v_stand_id IS NOT NULL THEN
    ...
    WHERE q.tenant_id = p_tenant_id
      ...
      AND NOT EXISTS (
        SELECT 1
        FROM ride_offers ro_any
        WHERE ro_any.tenant_id = p_tenant_id
          AND ro_any.ride_id   = p_ride_id
          AND ro_any.driver_id = q.driver_id
      )
      /* ✅ NUEVO: no re-seleccionar al mismo driver por ventana corta */
      AND NOT EXISTS (
        SELECT 1
        ...
        WHERE ro2.tenant_id = p_tenant_id
          ...
          AND ro2.sent_at  >= DATE_SUB(NOW(), INTERVAL (v_expires_sec + 1) SECOND)
      )
      AND (
        SELECT COUNT(*)
...
SQL);
    ...
    {
        // Si quieres restaurar una versión previa aquí, dímela y te la dejo.
        DB::unprepared('DROP PROCEDURE IF EXISTS sp_offer_wave_prio_v3');
    }
};
