...
return new class extends Migration
{
    ...
    {
        // 1) Drop (puede fallar si el DEFINER es root y el usuario no puede; en ese caso dropea con sudo mysql una vez)
        DB::unprepared("DROP PROCEDURE IF EXISTS sp_offer_wave_prio_v3");

        // 2) Create (sin DELIMITER; PDO envía el string completo al servidor)
        DB::unprepared(<<<'SQL'
...
wave_flow: BEGIN
  ...
  DECLARE v_expires_sec INT DEFAULT 300;
  DECLARE v_radius_km DOUBLE DEFAULT 3.00;
  ...
  /* =========================
   * Grupo 1: cola del stand (BASE-ONLY)
   * ========================= */
  ...
  IF v_stand_id IS NULL THEN
    ...
    /* Grupo 2B: fill con busy/on_ride si faltan */
