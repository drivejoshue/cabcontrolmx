@extends('layouts.admin_onboarding')
...
<div class="d-flex align-items-start justify-content-between mb-3">
  <div>
    <h1 class="h3 mb-1">Bienvenido, {{ $tenant->name }}</h1>
    ...
    <div class="small mt-1">
      ...
      <strong id="uiSavedCity">{{ $tenant->public_city ?: '—' }}</strong>
      @if($isReady)
  ...
  <div class="d-flex gap-2">
    <a href="{{ route('admin.dashboard') }}"
       class="btn btn-primary {{ $isReady ? '' : 'disabled' }}"
       @if(!$isReady) aria-disabled="true" tabindex="-1" @endif>
      Continuar
    </a>
  </div>
</div>
      ...
      <div class="card-body">
        ...
        <div class="row g-3">
          <div class="col-12 col-lg-4">
            <form method="POST" action="{{ route('admin.onboarding.location') }}" id="frmLocation">
              @csrf
              ...
              <input type="hidden" name="public_city" id="public_city" value="{{ old('public_city', $tenant->public_city) }}">
              ...
              <div class="mb-2">
                ...
                <input type="range"
                       ...
                       value="{{ old('coverage_radius_km', $tenant->coverage_radius_km ?? 8) }}">
              ...
              <input type="hidden" name="latitud" id="latitud" value="{{ old('latitud', $tenant->latitud) }}">
              <input type="hidden" name="longitud" id="longitud" value="{{ old('longitud', $tenant->longitud) }}">
    ...
    <div class="card">
      <div class="card-body">
        ...
        <div class="d-grid gap-2 mb-3">
          @php $canGo = $isReady; @endphp

          @if(isset($opsUrl) && $opsUrl)
            <a href="{{ $opsUrl }}"
               class="btn btn-outline-primary {{ $canGo ? '' : 'disabled' }}"
               @if(!$canGo) aria-disabled="true" tabindex="-1" @endif>
              Abrir Despacho
            </a>
          @else
          ...
          <a href="{{ route('admin.tenant.edit') }}"
             class="btn btn-outline-secondary {{ $canGo ? '' : 'disabled' }}"
             @if(!$canGo) aria-disabled="true" tabindex="-1" @endif>
            Ir a "Mi central"
          </a>
        </div>
        ...
        <div class="row g-3">
          <div class="col-12">
            <div class="border rounded p-3">
              ...
              @if(!empty($driverAppUrl))
                <div class="small text-muted mb-2">
                  <a href="{{ $driverAppUrl }}" target="_blank" rel="noopener">{{ $driverAppUrl }}</a>
                </div>
              @else
          ...
          <div class="col-12">
            <div class="border rounded p-3">
              ...
              @if(!empty($passengerAppUrl))
                <div class="small text-muted mb-2">
                  <a href="{{ $passengerAppUrl }}" target="_blank" rel="noopener">{{ $passengerAppUrl }}</a>
                </div>
              @else
...
(function () {
  function boot() {
    ...
    // Defaults
    const defaultLat = {{ (float)($tenant->latitud ?? 19.4326) }};
    const defaultLng = {{ (float)($tenant->longitud ?? -99.1332) }};
    ...
    if (citySelectEl) {
      const ts = new TomSelect(citySelectEl, {
        ...
        load: function(query, callback) {
          ...
          fetch(`{{ route('admin.onboarding.cities') }}?q=${encodeURIComponent(query)}`, {
