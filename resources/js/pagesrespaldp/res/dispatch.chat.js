...
// Todos deben aceptar tenant_id en querystring.
// ============================================================================

import { getTenantId, jsonHeaders, escapeHtml } from './dispatch.core.js';
import { fmtHM12_fromDb } from './dispatch.core.js';
...
export const ChatInbox = (() => {
  ...
  function notifyWarn(message) {
    try {
      if (typeof window.SmartNotifications !== 'undefined' && window.SmartNotifications?.show) {
        window.SmartNotifications.show('warning', String(message || ''));
        return;
      }
    } catch (_) {}
  ...
  function cacheEls() {
    els.openers = [...document.querySelectorAll('#btnChatInbox, [data-cc-chat-open="1"]')];
  ...
  function renderThreads() {
    ...
    els.threadsList.innerHTML = filtered.map(t => {
      ...
      const unread = t.unread_count > 0
        ? `<span class="cc-chat-unread">${t.unread_count > 99 ? '99+' : t.unread_count}</span>`
        : '';
      const time = t.last_at ? fmtMsgTime(t.last_at) : '';
      const last = escapeHtml(t.last_text || '');
  ...
  async function refreshMessages({ force = false, silent = true } = {}) {
    ...
    try {
      ...
      if (force || !state.lastMessageId) state.messages = msgs;
      else if (msgs.length) state.messages = [...state.messages, ...msgs];
...
export function initChatInbox({ exposeGlobal = false } = {}) {
  ChatInbox.init();
  if (exposeGlobal) window.ChatInbox = ChatInbox;
  return ChatInbox;
}
